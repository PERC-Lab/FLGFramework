...
/**
 * This class implements the quantitative FLG evaluation algorithm.
 * 
 * @author sghanava, gunterm, aminrabinia
 * 
 */
public class QuantitativeGRLStrategyAlgorithm extends PropagationGRLStrategyAlgorithm implements IGRLStrategyAlgorithm {
    ...
    public int getEvaluation(IntentionalElement element) {
        ...
        if(element.getLinksSrc() != null) {
        	Iterator it = element.getLinksDest().iterator(); // Return the list of elementlink
       
        
        	int i=0;
        	while(it.hasNext()) {
        		i++;		// FLG: counts the number of branches for each node
        		it.next();
        	}
        		it = element.getLinksDest().iterator(); 
        	
        		while (it.hasNext()) { 
        		ElementLink link = (ElementLink) it.next();
        		if (link instanceof Decomposition && i>1 ) {          // FLG: if branches are more than one, it does the decomposition eval; otherwise it blocks the propagation of evaluation of single nodes  
	...
	protected int getANDEvalRecursive(IntentionalElement elm, int val) {		
		// FLG: AND sequence evaluation
		//check if element has a child and has AND decomposition 
		if(!elm.getLinksDest().isEmpty() 
				&& elm.getDecompositionType().getValue()==DecompositionType.AND) {		
			Iterator itr= elm.getLinksDest().iterator();
			ElementLink lnk=(ElementLink) itr.next();
			elm= (IntentionalElement)lnk.getSrc(); //gets its child
			val=evaluateDependency(val,lnk); //updates value of AND decomposition 
			return	getANDEvalRecursive(elm,val); //recursion with new val and child
		} //if no child or AND decomp., return the last update of decomp. value
			return val;
	}
    ...
    protected int evaluateDecomposition(IntentionalElement element, int decompositionValue, Iterator it, ElementLink link) {
    	// FLG: edited 
            ...
            if (decompositionValue < minRange) {
            	...
            	if(decompositionValue < getANDEvalRecursive(((IntentionalElement)link.getSrc()),((Evaluation) evaluations.get(link.getSrc())).getEvaluation())){            
            		decompositionValue = getANDEvalRecursive(((IntentionalElement)link.getSrc()),((Evaluation) evaluations.get(link.getSrc())).getEvaluation());
            }
            
        } else if(element.getDecompositionType().getValue() == DecompositionType.XOR) {
        		if(decompositionValue < getANDEvalRecursive(((IntentionalElement)link.getSrc()),((Evaluation) evaluations.get(link.getSrc())).getEvaluation())){                
        		decompositionValue = getANDEvalRecursive(((IntentionalElement)link.getSrc()),((Evaluation) evaluations.get(link.getSrc())).getEvaluation());
        		} else if(decompositionValue == getANDEvalRecursive(((IntentionalElement)link.getSrc()),((Evaluation) evaluations.get(link.getSrc())).getEvaluation()))
        			decompositionValue = 0;
        }
